...
map.murderRate <- map.murderRate+  geom_map(map = us, aes(color=mergedData$Murder, fill = mergedData$Murder)) map.murderRate <- map.murderRate + expand_limits(x = US$LONG$StateCenterX, y = mergedData$StateCenterY)
